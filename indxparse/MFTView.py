        ...
        def add_node(mftfile: NTFSFile, record: MFTRecord) -> None:
            ...
            if record.magic() != 0x454C4946:
                if record.magic() == int("BAAD", 0x16):
                    node = Node(rec_num, "BAAD", None, record.is_directory())
            ...
            if record.inode == 0x5:
                ...
                # and creates a cycle
                node = Node(record.inode, fn.filename(), None, record.is_directory())
...
class MFTTreeCtrl(wx.TreeCtrl):
    ...
    def _extend(self, item):
        ...
        for child_node in sorted(
            [c for c in node.children if c.is_directory], key=lambda x: x.get_name()
        ):
        ...
        for child_node in sorted(
            [c for c in node.children if not c.is_directory], key=lambda x: x.get_name()
        ):
            child_item = self.AppendItem(item, child_node.get_name())
...
class RunlistPanel(wx.Panel):
    ...
    def update(self, event):
        ...
        coff = str(
            self._model.volume_offset() + self._offset * self._model.cluster_size()
        )
...
""",
        ...
        self._volume_offset_label = wx.TextCtrl(
            self, -1, self._volume_offset_label_ok, style=wx.TE_READONLY
        )

        hbox1.Add(self._volume_offset_label, 1, wx.EXPAND)
        self._volume_offset_text = wx.TextCtrl(
            self, -1, str(self._model.volume_offset())
        )
        ...
        self._cluster_size_label = wx.TextCtrl(
            self, -1, self._cluster_size_label_ok, style=wx.TE_READONLY
        )
        ...
        eventManager.Register(
            self._updated_volume_offset, EVT_VOLUME_OFFSET_UPDATED_EVENT, self._model
        )
        eventManager.Register(
            self._updated_cluster_size, EVT_CLUSTER_SIZE_UPDATED_EVENT, self._model
        )
...
class DataPane(wx.Panel):
    def __init__(self, *args, **kwargs):
        ...
        self._text.SetFont(wx.Font(8, wx.SWISS, wx.NORMAL, wx.NORMAL, False, "Courier"))
        ...
        self._strings.SetFont(
            wx.Font(8, wx.SWISS, wx.NORMAL, wx.NORMAL, False, "Courier")
        )
...
class RecordMetadataPane(RecordPane):
    ...
    def __init__(self, *args, **kwargs):
        ...
        record_box_sizer.Add(self._record_number, self.NOT_EXPAND_VERTICALLY, wx.EXPAND)

        self._record_attributes_line = LabelledLine(self, "Attributes", "<none>")
        record_box_sizer.Add(
            self._record_attributes_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
        )

        self._record_size_line = LabelledLine(self, "Size (bytes)", "")
        record_box_sizer.Add(
            self._record_size_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
        )

        self._seq_line = LabelledLine(self, "Sequence Number", "")
        record_box_sizer.Add(self._seq_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND)

        # note, must add the sizer, not the `record_box`
        self._sizer.Add(record_box_sizer, self.NOT_EXPAND_VERTICALLY, wx.EXPAND)
        ...
        si_box_sizer.Add(
            self._si_attributes_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
        )

        self._si_created_line = LabelledLine(self, "Created", "")
        si_box_sizer.Add(self._si_created_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND)

        self._si_modified_line = LabelledLine(self, "Modified", "")
        si_box_sizer.Add(self._si_modified_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND)

        self._si_changed_line = LabelledLine(self, "Changed", "")
        si_box_sizer.Add(self._si_changed_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND)

        self._si_accessed_line = LabelledLine(self, "Accessed", "")
        si_box_sizer.Add(self._si_accessed_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND)

        self._sizer.Add(si_box_sizer, self.NOT_EXPAND_VERTICALLY, wx.EXPAND)
        ...
        for a in [3, 1, 2, 0]:  # order so that most common at top
            ...
            self._fn[a].box = wx.StaticBox(
                self, -1, "Filename Information Attribute (%s)" % (filename_type)
            )
            fn_box_sizer = wx.StaticBoxSizer(self._fn[a].box, wx.VERTICAL)
            self._fn[a].name_line = LabelledLine(self, "Filename", "")
            fn_box_sizer.Add(
                self._fn[a].name_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
            )

            self._fn[a].attributes_line = LabelledLine(self, "Attributes", "<none>")
            fn_box_sizer.Add(
                self._fn[a].attributes_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
            )

            self._fn[a].alloc_size_line = LabelledLine(
                self, "Allocated Size (bytes)", ""
            )
            fn_box_sizer.Add(
                self._fn[a].alloc_size_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
            )

            self._fn[a].log_size_line = LabelledLine(self, "Logical Size (bytes)", "")
            fn_box_sizer.Add(
                self._fn[a].log_size_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
            )

            self._fn[a].created_line = LabelledLine(self, "Created", "")
            fn_box_sizer.Add(
                self._fn[a].created_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
            )

            self._fn[a].modified_line = LabelledLine(self, "Modified", "")
            fn_box_sizer.Add(
                self._fn[a].modified_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
            )

            self._fn[a].changed_line = LabelledLine(self, "Changed", "")
            fn_box_sizer.Add(
                self._fn[a].changed_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
            )

            self._fn[a].accessed_line = LabelledLine(self, "Accessed", "")
            fn_box_sizer.Add(
                self._fn[a].accessed_line, self.NOT_EXPAND_VERTICALLY, wx.EXPAND
            )

            self._sizer.Add(fn_box_sizer, self.NOT_EXPAND_VERTICALLY, wx.EXPAND)
    ...
    def update(self, event):
        ...
        crtime = (
            self._model.record().standard_information().created_time().isoformat("T")
        )
        self._si_created_line.update(crtime + "Z")

        mtime = (
            self._model.record().standard_information().modified_time().isoformat("T")
        )
        self._si_modified_line.update(mtime + "Z")

        chtime = (
            self._model.record().standard_information().changed_time().isoformat("T")
        )
        self._si_changed_line.update(chtime + "Z")

        atime = (
            self._model.record().standard_information().accessed_time().isoformat("T")
        )
                    ...
                    if attr.non_resident():
                        try:
                            for offset, length in attr.runlist().runs():
                                runlist_panel = RunlistPanel(
                                    self, offset, length, self._model
                                )
                                self._sizer.Add(runlist_panel, 0, wx.EXPAND)
                        except IndexError:
                            sys.stderr.write("Error parsing runlist\n")
                            continue
                    elif len(attr.value()) > 0:
                        value_view = wx.TextCtrl(self, style=wx.TE_MULTILINE)
                        value_view.SetFont(
                            wx.Font(8, wx.SWISS, wx.NORMAL, wx.NORMAL, False, "Courier")
                        )
...
class RecordAttributePane(RecordPane):
    ...
    def update(self, event):
        ...
        for attr in self._model.record().attributes():
            try:
                at_view = wx.StaticBox(self, -1, "Attribute, type " + hex(attr.type()))
                at_view_sizer = wx.StaticBoxSizer(at_view, wx.VERTICAL)

                at_view_sizer.Add(
                    LabelledLine(self, "Type", str(attr.type())),
                    self.NOT_EXPAND_VERTICALLY,
                    wx.EXPAND,
                )
                at_view_sizer.Add(
                    LabelledLine(self, "Reported Name", attr.name()),
                    self.NOT_EXPAND_VERTICALLY,
                    wx.EXPAND,
                )
                at_view_sizer.Add(
                    LabelledLine(self, "Type Name", attr.TYPES[attr.type()]),
                    self.NOT_EXPAND_VERTICALLY,
                    wx.EXPAND,
                )
                ...
                if len(attributes) > 0:
                    at_view_sizer.Add(
                        LabelledLine(self, "Attributes", ", ".join(attributes)),
                        self.NOT_EXPAND_VERTICALLY,
                        wx.EXPAND,
                    )
                else:
                    at_view_sizer.Add(
                        LabelledLine(self, "Attributes", "<none>"),
                        self.NOT_EXPAND_VERTICALLY,
                        wx.EXPAND,
                    )

                at_view_sizer.Add(
                    LabelledLine(self, "Size", str(attr.size())),
                    self.NOT_EXPAND_VERTICALLY,
                    wx.EXPAND,
                )
                if attr.non_resident():
                    at_view_sizer.Add(
                        LabelledLine(self, "Resident", "False"),
                        self.NOT_EXPAND_VERTICALLY,
                        wx.EXPAND,
                    )
                else:
                    at_view_sizer.Add(
                        LabelledLine(self, "Resident", "True"),
                        self.NOT_EXPAND_VERTICALLY,
                        wx.EXPAND,
                    )
                ...
                self._sizer.Add(
                    at_view_sizer, self.EXPAND_VERTICALLY, wx.ALL | wx.EXPAND
                )
            except ZeroDivisionError:
                continue
        self.SetAutoLayout(1)
    ...
    def update(self, event):
        ...
        if indxroot and indxroot.non_resident() == 0:
            ...
            for e in irh.node_header().entries():
                ...
                ir_view_sizer.Add(
                    LabelledLine(self, "Filename", e.filename_information().filename()),
                    self.NOT_EXPAND_VERTICALLY,
                    wx.EXPAND,
                )
                ir_view_sizer.Add(
                    LabelledLine(
                        self,
                        "Size (bytes)",
                        str(e.filename_information().logical_size()),
                    ),
                    self.NOT_EXPAND_VERTICALLY,
                    wx.EXPAND,
                )
                ir_view_sizer.Add(
                    LabelledLine(
                        self,
                        "Created",
                        e.filename_information().created_time().isoformat("T") + "Z",
                    ),
                    self.NOT_EXPAND_VERTICALLY,
                    wx.EXPAND,
                )
                ir_view_sizer.Add(
                    LabelledLine(
                        self,
                        "Modified",
                        e.filename_information().modified_time().isoformat("T") + "Z",
                    ),
                    self.NOT_EXPAND_VERTICALLY,
                    wx.EXPAND,
                )
                ir_view_sizer.Add(
                    LabelledLine(
                        self,
                        "Changed",
                        e.filename_information().changed_time().isoformat("T") + "Z",
                    ),
                    self.NOT_EXPAND_VERTICALLY,
                    wx.EXPAND,
                )
                ir_view_sizer.Add(
                    LabelledLine(
                        self,
                        "Accessed",
                        e.filename_information().accessed_time().isoformat("T") + "Z",
                    ),
                    self.NOT_EXPAND_VERTICALLY,
                    wx.EXPAND,
                )
                self._sizer.Add(
                    ir_view_sizer, self.NOT_EXPAND_VERTICALLY, wx.ALL | wx.EXPAND
                )
        ...
        for attr in self._model.record().attributes():
            ...
            if attr.non_resident() != 0:
                ...
                for offset, length in attr.runlist().runs():
                    rl_view_sizer.Add(
                        RunlistPanel(self, offset, length, self._model),
                        self.NOT_EXPAND_VERTICALLY,
                        wx.EXPAND,
                    )
                self._sizer.Add(
                    rl_view_sizer, self.NOT_EXPAND_VERTICALLY, wx.ALL | wx.EXPAND
                )
        self.SetAutoLayout(1)
        self.SetupScrolling()
...
class MFTFileViewer(wx.Frame):
    def __init__(self, parent, filename):
        super(MFTFileViewer, self).__init__(
            parent, -1, "MFT File Viewer", size=(900, 600)
        )
